...
def draw_board(board, screen, animations):
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            value = board[i][j]
            rect = pygame.Rect(j * CELL_SIZE + CELL_SIZE, i * CELL_SIZE + CELL_SIZE,CELL_SIZE,CELL_SIZE)
            pygame.draw.rect(screen, CELL_COLOR if value else CELL_EMPTY_COLOR, rect)
            if value:
                if(i , j) in animations:
                    #Animate numbers towards thier positions
                    current_x, current_y = animations[(i , j)]
                    target_x, target_y = j * CELL_SIZE + CELL_SIZE, i * CELL_SIZE + CELL_SIZE
